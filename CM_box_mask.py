    ...
    for i in range(imgs_gt.size(0)):
      ...
      for k in range(triples_i.shape[0]):
          ...
          if(triples_i[k][1] != 0):
              ...
              edge_gt = get_relationship(box_s, box_o, mask_s, mask_o)#get_relationship(objs_of_img, bbxs_of_img, masks_of_img, vocab)
...
#    break    
  ...
  title = 'M1_bm_'+ args.which_data + '_' + label[fn] 
  ax.set(title=title,
             ylabel='Predicted label',
             xlabel='True label')
  fig = ax.get_figure()
  filename = 'CM1_bm_' + fn + '_' + args.which_data + '.png'
  ...
  if got_checkpoint:
    ...
    for i, path in enumerate(checkpoint_list):
      if os.path.isfile(path):
        ...
        if loader is None:
          loader = build_loader(args, checkpoint)
        output_dir = os.path.join(args.output_dir, 'result_CM_bm')
